...
export default async function WhyWheelhouse({ locale }: { locale: string }) {
  const t = await getTranslations({ locale, namespace: 'WhyWheelhouse' });
  ...
  return (
    <section className="container-gutter py-block space-y-12">
      ...
      <div>
        <h2 className="text-2xl font-semibold mb-6">{t('whyTitle')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white dark:bg-neutral-900 rounded-lg shadow p-6 flex flex-col items-start">
              <div className="text-lg font-medium mb-2">{t(`item${i}Title`)}</div>
