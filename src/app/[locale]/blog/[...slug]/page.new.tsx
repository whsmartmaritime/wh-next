...
export async function generateMetadata({
  ...
  // Tách category và slug từ params
  const { locale, slug } = params;
  const [category, postSlug] = slug;
  if (!category || !postSlug) return {};
  ...
  const languages = post.hasTranslation
    ? {
        [post.locale]: new URL(
          `/${post.locale}/blog/${category}/${post.slug}`,
          base
        ),
        [post.locale === "en" ? "vi" : "en"]: new URL(
          `/${post.locale === "en" ? "vi" : "en"}/blog/${category}/${
  ...
  return {
    ...
    alternates: {
      canonical: new URL(`/${locale}/blog/${category}/${postSlug}`, base),
      languages,
    },
    openGraph: {
      title: post.title,
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  if (!category || !postSlug) notFound();

  // Lấy bài viết và translations
  const [post, t] = await Promise.all([
    getPost(postSlug),
    getTranslations({ locale, namespace: "blog" }),
  ]);

  // Kiểm tra nếu không tìm thấy bài viết hoặc category không khớp
