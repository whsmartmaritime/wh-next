      ...
      <ul className="flex space-x-8">
        {menuItems.map((item, index) => (
          <li key={index} className="group relative overflow-visible">
            ...
            {item?.hasDropdown && (
              <div className="fixed top-[90px] left-0 right-0 w-full opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[60] bg-white dark:bg-black shadow-lg">
                <div className="container-gutter grid grid-cols-12 mx-auto px-4 py-8">
                  ...
                  {item?.dropdownContent?.columns?.map((column, colIndex) => (
                    <div
                      key={colIndex}
                      className={`space-y-4 ${
                        colIndex === 0 ? "col-span-2" : "col-span-3"
                      }`}
                    >
                  ...
                  {item?.ctaContent && (
                    <div className="col-span-4 space-y-4">
                      ...
                      <Link
                        ...
                        className="no-underline flex items-center gap-2 text-lg font-bold hover:opacity-80 transition-opacity"
                      >
